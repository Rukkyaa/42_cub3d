...
int	handle_doors_x(t_game *game, t_raycast_data *d, t_door *door,
		int *tile_found)
{
	t_vector3d	door_intersection;
	float		door_offset;

	door_offset = 32 * d->v_ray_dir.y / fabs(d->v_ray_dir.x) - door->open_state;
	...
	if (((int)door_intersection.y) / 64 == d->v_map_check.y)
	{
		...
		d->collision.orientation = get_collision_orientation(d->last_step,
				d->v_step);
...
int	handle_doors_y(t_game *game, t_raycast_data *d, t_door *door,
		int *tile_found)
{
	t_vector3d	door_intersection;
	float		door_offset;
	...
	if (d->v_map_check.x * 64 < door_intersection.x
		&& door_intersection.x < (d->v_map_check.x + 1) * 64)
	{
...
int	handle_doors(t_game *game, t_raycast_data *d, int *tile_found)
{
	...
	if (door->x_oriented)
		...
		return (handle_doors_y(game, d, door, tile_found));
}
