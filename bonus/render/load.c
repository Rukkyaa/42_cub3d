...
/*   Updated: 2023/03/08 12:12:22 by axlamber         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (++y < map_heigth(game->map) && y < y_end)
	{
		...
		if (x != -1 && x != map_width(game->map) - 8)
		{
			...
			if (is_wall(game->map[y][x]))
				draw_filled_rectangle(pos, size, BLUE_PIXEL);
			else
				draw_filled_rectangle(pos, size, GREEN_PIXEL);
			pos.x = 32 - (int)game->player.pos.x % 64 / 2;
		}
		while (++x < (map_width(game->map)) && x < x_end)
		{
			if (is_wall(game->map[y][x]))
				draw_filled_square(pos, 32, BLUE_PIXEL);
			else if (game->map[y][x] == '0')
				draw_filled_square(pos, 32, GREEN_PIXEL);
			else
				draw_filled_square(pos, 32, 888888);
			pos.x += 32;
		}
